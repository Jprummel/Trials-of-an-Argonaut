...
function RateAnnouncement( rateURL, gid, bVoteUp )
{
	...
	.fail( function( jqxhr ) {
		...
		{
			case 21:
				ShowAlertDialog( 'Fout', 'Je moet aangemeld zijn om dit te kunnen doen.' );
				break;
			case 24:
				ShowAlertDialog( 'Fout',
					'Je account voldoet niet aan de vereisten om deze functie te gebruiken. Bezoek <a class="whiteLink" href="https://support.steampowered.com/kb_article.php?ref=3330-IAGK-7663" target="_blank">Steam Support</a> voor meer informatie.'
				);
				break;
			case 15:
				ShowAlertDialog( 'Fout', 'Je hebt geen toestemming om die actie uit te voeren.' );
				break;
			default:
				ShowAlertDialog( 'Fout', 'Er is een fout opgetreden bij het uitvoeren van je verzoek: ' + responseJSON.success );
				break;
		}
	} );
	return false;
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	if ( scrollTopPrevious != 0 )
	{
		...
		else
		{
			...
			{
				this.m_LoadingDialog = ShowBlockingWaitDialog( 'Een ogenblik geduld', 'Je wordt nu terugverwezen naar waar je vandaan kwam...' );
			}
			window.scrollTo( 0, scrollTopPrevious );
		}
	}
...
function ToggleFamilyView( bLocked, strURL )
{
	...
	{
		ShowConfirmDialog( 'Terugkeren naar gezinsweergave',
			'Weet je zeker dat je terug wilt keren naar de gezinsweergave?',
			'Terugkeren naar gezinsweergave'
		).done( function() {
			ShowBlockingWaitDialog( 'Terugkeren naar gezinsweergave' );
			window.location = strURL;
		});
	}
}
