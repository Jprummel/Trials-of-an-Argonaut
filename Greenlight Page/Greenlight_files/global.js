...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Geef een reden op voor het melden van misbruik' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Overtreding melden', 'Er is een probleem opgetreden bij het opslaan van je rapport. Probeer het later opnieuw.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Let op: de link waar je op hebt geklikt leidt niet naar een officiële Steam-website.\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ 'Geef nooit je gebruikersnaam of wachtwoord op als deze website daarnaar vraagt. Je zou je Steam-account ermee kunnen verliezen - en daarmee ook al je games!\n'
						+ 'Weet je zeker dat je deze pagina wilt bezoeken? Klik op OK om op eigen risico door te gaan.\n' );
	}

	ShowAlertDialog( '', 'De URL heeft een ongeldig formaat.');
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Overtreding melden', g_AbuseModalContents );
	}
}
	...
	var fnDataFactory = function( key ) {
		...
		if ( rgItemKey.length >= 3 && rgItemKey.length <= 5 )
		{
			...
			else
			{
				...
				strURL += '?content_only=1&omit_owner=1&l=dutch';
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Taal wijzigen', 'Er is een fout opgetreden bij het verbinden met de Steam-servers. Probeer het later opnieuw.' );
		});
}
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Abonneren op discussie', 'Er is een probleem opgetreden bij het bijwerken van je abonnement. Probeer het later opnieuw.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Abonneren op discussie', 'Er is een probleem opgetreden bij het bijwerken van je abonnement. Probeer het later opnieuw.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Voer een bericht in om te kunnen plaatsen.' );
			return;
		}
	...
	{
		var strMessage = 'Sorry, er is een fout opgetreden: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Er is een fout opgetreden bij het communiceren met het netwerk. Probeer het later opnieuw.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Abonneren op discussie', 'Je zult een melding ontvangen wanneer iemand hierop reageert.');
			}, function() {
				ShowAlertDialog('Abonneren op discussie', 'Er is een probleem opgetreden bij het bijwerken van je abonnement. Probeer het later opnieuw.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Uitschrijven van reacties', 'Je zult niet langer meldingen ontvangen van reacties hierop.');
			}, function() {
				ShowAlertDialog('Uitschrijven van reacties', 'Er is een probleem opgetreden bij het bijwerken van je abonnement. Probeer het later opnieuw.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Tekstopmaak', data );
	});
};
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Delen', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = ShowDialog( 'Delen', $( 'ShareOnSteamDialog' ) );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			gShareOnSteamDialog.Dismiss();
			ShowAlertDialog( 'Delen', 'De statusupdate is geplaatst bij je vriendenactiviteit.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Delen', 'Er is een fout opgetreden bij het delen van de statusupdate. Probeer het later nog eens.' );
		}
	});
}
